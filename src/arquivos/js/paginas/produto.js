...
var paginaProduto = {
	...
	'selecaoSkus': function (elemento,skuJsonAdultered) {
	...
	'botaoDeCompra': function (elemento) {
	...
	'aviseme': function (elemento) {
		var opcoes = {
			"titulo": '<span>Que pena, esse tamanho está indisponível, mas nós te avisamos quando chegar!</span>',
	...
	'exibirVariacaoDeCores': function(){
		var urlSimilares = "/api/catalog_system/pub/products/crossselling/similars/" + window.skuJson.productId;
		...
		jqXHR.done(function sucesso(value) {
			if (value.length > 0) {
				...
				var titulo = $('<div />', {class:'titulo', text:'Escolha a cor:'}).appendTo(variacaoSku);
				var ul = $('<ul />').appendTo(variacaoSku);

				var produtosJaExibidos = [];
				for (var i = 0; i < value.length; i++){

					if($.inArray(value[i].productId, produtosJaExibidos) == -1){
						produtosJaExibidos.push(value[i].productId);
						var li = $('<li />');
						var link = $('<a />',{href:value[i].link,title:value[i].productName }).appendTo(li);
						var img = value[i].items[0].images[0].imageTag;
						img = img.replace(/#width#/gi,"90").replace(/#height#/gi,"90");
						img = img.replace("~","//"+ loja.accontuName +".vteximg.com.br/");
						$(img).appendTo(link);
	...
	'shortDescription': function(){
		$('.descricao-produto a').on('click', function(event){
			...
			$('html,body').animate({'scrollTop':$('#descricao-completa').eq(0).offset().top-50},500);
		});
	},
	'zoomImagemPrincipal':function(){

		if( !utils.isMobile ){
			// $("body").on('click', '.sku-rich-image-main', function(event){
			// 	event.preventDefault();
			// });


			$('.product-image .apresentacao #include').on('mouseover',function(){
				var srcImg = $('.sku-rich-image-main').attr('src');

				$('.sku-rich-image-main').attr('src',
	...
	'ordenarEspecificacoes':function(skuJson){
		...
		skuJsonAdultered.dimensionsMap['Tamanhos'] = ordenarEspecificacoes(
			skuJsonAdultered.dimensionsMap['Tamanhos'],
			['PP','P','M','G','GG']
		);

		return skuJsonAdultered;
	},
	'imagensCompreJunto': function(){
		window.onload = function() {
			setTimeout(function(){
				...
				for(let i = 0; i < images.length; i++){
					...
					let newUrl = utils.alterarTamanhoImagemSrcVtex(imageUrl, 250, 250);
