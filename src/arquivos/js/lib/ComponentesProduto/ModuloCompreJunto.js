...
const { alterarTamanhoImagemSrcVtex } = require("./main");
/**
 * Modulo de quantidade
 * Permite escolher a quantidade de um sku
 */
var ModuloCompreJunto = function (elemento='.compre-junto') {
	Modulo.call(this,elemento);
	this._opcoes = {
		titulo: "Compre junto"
	};
};
exports.ModuloCompreJunto = ModuloCompreJunto;
	...
	function sucesso(htmlCJVtex, textStatus, jqXHR) {
		if (htmlCJVtex.trim().length > 1) {
			...
			for (var i = 0; i < imagens.length; i++) {
				imagens[i].setAttribute('src', alterarTamanhoImagemSrcVtex(imagens[i].src, 265, 403));
			}
