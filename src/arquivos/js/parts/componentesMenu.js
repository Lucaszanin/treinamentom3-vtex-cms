...
var menuContents = {
	'getContent': function(){
		...
		$.get(url, function(data){
			...
			$('<div/>', {class:'elementos-menu'}).appendTo('.menu-principal .itens >li .container');

			// menuContents.insertProduct($conteudo);
			menuContents.insertBanners($conteudo);
		});
	},
	'insertProduct': function($conteudo){
		...
		if(shelfs){
			...
			for(let i = 0; i < categories.length; i++){
				...
				let productMenuContainer = $('.menu-principal a[title*="'+categoryName+'"]').parent().find('.elementos-menu');
	...
	'insertBanners': function($conteudo){
		...
		if(banners){
			...
			for(let i = 0; i < imgs.length; i++){
				...
				let bannerMenuContainer = $('.menu-principal a[title*="'+categoryName+'"]').parent().find('.elementos-menu');
