export default class CrossSelling{
	...
	similars(title=""){
		try {
			this._loadProduct("similars")
				.done(( products, textStatus, jqXHR )=>{
	...
	_desenhar(products,type,title="") {
		if (products.length > 0) {
			...
			let _html = `<div class="cross-selling ${type.toLowerCase()}">
				...
				<ul class="lista">
					${products.map((item,i)=>{
						let imgTag = products[i].items[0].images[0].imageTag;
						imgTag = imgTag.replace(/#width#/gi, "90")
							.replace(/#height#/gi, "90")
							.replace("~","");

						return `<li class="item ${item.productId == this._idProduct?"selected":''}">
